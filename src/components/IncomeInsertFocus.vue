<template>
  <div class="incomeInsertFocus">
    <div id="incomeInsertFocus" class="modal fade">
            ...
            <div class="modal-header">
              <div class="row w-100">
                <div class="col-auto me-5">
                  <h3 class="fw-light">
                    <i class="bi bi-cash-stack"></i><i class="bi bi-headphones sub"></i>
                    <span class="pe-3 me-3 border-right">収入登録</span>FOCUS
                  </h3>
                </div>
                <div class="col-auto">
                  <div class="row">
            ...
            <div class="modal-body">
              ...
              <div class="row gx-5" v-if="show">
                <div class="col-6 mb-5" v-for="n of colNum" v-bind:key="n">
                  <div class="row">
                    <div class="col-8">
                      <label v-bind:for="'focusCustomer' + n"><i class="bi bi-building"></i> 取引先</label>
                      <select v-bind:id="'focusCustomer' + n" class="form-select form-select-sm">
                        <option value="">選択...</option>
                        <option v-bind:value="item.customerId" v-for="item of customers" v-bind:key="item.customerId">
                          {{ item.customerName }}
                        </option>
                      </select>
                    ...
                    <div class="col-4">
                      <label v-bind:for="'focusIncomeYear' + n"><i class="bi bi-calendar"></i> 年</label>
                      <input maxlength="4" v-bind:id="'focusIncomeYear' + n" class="form-control form-control-sm"
                        placeholder="数字4桁" />
...
export default {
  ...
  methods: {
    ...
    async insert() {
      ...
      const isInvalid = function () {
        // eventで拾ったチェックエラー検証
        let bool = false;
        const focus = document.querySelector("#incomeInsertFocus");
        const es = focus.querySelectorAll(".focus-input");
        es.forEach((e) => {
          if (e.classList.contains("is-invalid")) bool = true;
        });
        const customers = focus.querySelectorAll("[id^='focusCustomer']");
        customers.forEach((e) => {
          if (e.classList.contains("is-invalid")) bool = true;
        });
        const years = focus.querySelectorAll("[id^='focusIncomeYear']");
        years.forEach((e) => {
          if (e.classList.contains("is-invalid")) bool = true;
        });
        return bool;
      ...
      amountValues.forEach((amount, index) => {
        const year = document.getElementById("focusIncomeYear" + (index + 1));
        const customer = document.getElementById("focusCustomer" + (index + 1));
        const arrEach = [];
        const dictEach = {
          customer: customer.value,
          customerElem: customer,
          year: year.value,
          yearElem: year,
        };
    ...
    async checkValues(arr) {
      ...
      for (let docs of arr) {
        // 支出項目と年がどちらかしか入っていない
        if (!docs.year && docs.customer) {
          docs.yearElem.classList.add("is-invalid");
          result = true;
          continue;
        } else if (docs.year && !docs.customer) {
          docs.customerElem.classList.add("is-invalid");
          result = true;
          continue;
        }
        let cnt = this.rowNum.length;
        for (let doc of docs.content) {
          if (docs.year && docs.customer && !doc.date && !doc.amount) {
            cnt--;
          } else if (docs.year && docs.customer) {
        ...
        if (cnt == 0) {
          const customer = await window.call.getCustomerDataById(Number(docs.customer));
          errMsgs.push(
            `支出項目:${customer.customerName}(${docs.year}年)の表について、日付と金額を入力してください。`
          );
          result = true;
        }
      }
      ...
      arr.forEach((elem) => {
        ...
        elem.content.forEach((e) => {
          const item = {
            ...
            incomeAmount: Number(e.amount),
          };
          arrIns.push(item);
        });
      });

      return await window.call.insertIncome(arrIns);
    },
    clear() {
  ...
  mounted() {
    ...
    const modal = document.getElementById("incomeInsertFocus");
  ...
  updated() {
    ...
    const listElemsCheck = function (target) {
      ...
      if (/amount-./.test(target.id)) {
        ...
        const yElem = document.getElementById(
          `focusIncomeYear${target.id.split("-")[1]}`
        );
    ...
    years.forEach((elem) => {
      elem.addEventListener("focusout", (event) => {
        ...
        const num = event.target.id.replace("focusIncomeYear", "");
        const exItem = document.getElementById("focusCustomer" + num);
    ...
    const exItems = focus.querySelectorAll("[id^='focusCustomer']");
    exItems.forEach((elem) => {
      elem.addEventListener("change", (event) => {
        const num = event.target.id.replace("focusCustomer", "");
        const exYear = document.getElementById("focusIncomeYear" + num);
